...
Transform::update()
{
  auto p = parent(); // pai do objeto de cena que possui este transform
  ...
  _inverseMatrix = inverseLocalMatrix() * p->_inverseMatrix;
	...
	// DONE: update the transform of all scene object's children.
	for (auto it = sceneObject()->beginChildren(); it != sceneObject()->endChildren(); it++)
		(*it)->transform()->update();
  changed = true;
}
