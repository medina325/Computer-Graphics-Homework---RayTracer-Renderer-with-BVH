...
// ===========
	...
	void
		SceneObject::setLight(const Reference<Light>& l)
	{
		if (l != nullptr)
		{
			if (findComponent<Light>() == nullptr)
			{
				_light = l;
				addComponent(_light);

				// Add light to the list of lights being used
	...
	{
		// Remove recursively child's children's primitives
		...
		if (child->findComponent<Light>() != nullptr)
			_scene->removeLight(child->light());
	}
